...
public class HomePageActivity extends Activity implements
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		backButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				Intent backIntent = new Intent(HomePageActivity.this,
						LoginActivity.class);
				...
				if (position == 0) {
					...
					if (!TextUtils.isEmpty("imei")) {
						...
						Log.i(TAG, "deflutStudentInfo.name "
								+ deflutStudentInfo.name);
						Log.i(TAG, "deflutStudentInfo.imei "
								+ deflutStudentInfo.imei);
						Log.i(TAG, "deflutStudentInfo.id "
								+ deflutStudentInfo.id);

						intent.putExtra("imei", deflutStudentInfo.imei);
						startActivity(intent);
					}
		...
		if (curretUserInfo.roleType == UserType.ROLE_PARENT) {
			...
			mProgressDialog.setOnCancelListener(new OnCancelListener() {
				...
				public void onCancel(DialogInterface dialog) {
					...
					// TODO Auto-generated method stub
					if (mGetAllChildTask != null
							&& !mGetAllChildTask.isCancelled()) {
	...
	public class ImageAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (convertView == null) {
				convertView = inflater.inflate(R.layout.gridview_picture_item,
						null);
				...
				viewHolder.numberText = (TextView) findViewById(R.id.number_text);
		...
		private Integer[] teacherPictures = { R.drawable.homework,
				R.drawable.message, R.drawable.check, R.drawable.officialweb,
				R.drawable.change_password };
		private Integer[] teacherTitles = { R.string.home_work,
				R.string.system_note, R.string.attendance_information,
				R.string.official_site, R.string.change_password };
		private Integer[] parentPictures = { R.drawable.homework,
				R.drawable.message, R.drawable.check, R.drawable.officialweb,
				R.drawable.change_password, R.drawable.position };
		private Integer[] parentTitles = { R.string.home_work,
				R.string.system_note, R.string.attendance_information,
				R.string.official_site, R.string.change_password,
	...
	public class GetAllClassTask extends AsyncTask<Void, Void, String> {
		...
		protected String doInBackground(Void... params) {
			...
			try {
				schoolsInfo = RestClient.getClassInfos(curretUserInfo.id,
						curretUserInfo.ticket, curretUserInfo.fkClassId);
				...
				try {
					if (JSONParser.getIntByTag(schoolsInfo,
						...
						if (!TextUtils.isEmpty(errorMessage)) {
							Toast.makeText(HomePageActivity.this, errorMessage,
									Toast.LENGTH_LONG).show();
						} else {
							Toast.makeText(HomePageActivity.this, schoolsInfo,
									Toast.LENGTH_LONG).show();
						}
					}
				} catch (JSONException e) {
	...
	public class GetAllNoteTask extends AsyncTask<Void, String, String> {
		...
		protected String doInBackground(Void... params) {
			...
			try {
				...
				if (curretUserInfo.roleType == UserInfo.UserType.ROLE_TEACHER) {
					...
					info.append("\"" + deflutStudentInfo.id + "\"");
					info.append(",");
				}
				...
				homeWorksInfo = RestClient.getHomeWorks(curretUserInfo.id,
						curretUserInfo.ticket,
						curretUserInfo.roleType.toString(), info.toString(), 1,
						5);
			} catch (ConnectTimeoutException stex) {
			...
			if (!TextUtils.isEmpty(homeWorksInfo)) {
				...
				try {
					if (JSONParser.getIntByTag(homeWorksInfo,
							RestClient.RESULT_TAG_CODE) == RestClient.RESULT_TAG_SUCCESS) {

						homeWorkTotal = JSONParser.getIntByTag(homeWorksInfo,
								"total");
						// try {
						// if(JSONParser.getStringByTag(homeWorksInfo,
						// "datas")!=null){
			...
			Log.i(TAG, "schoolsInfo1 " + homeWorksInfo);
			...
			try {
				...
				messageInfos = RestClient.getMessageInfos(curretUserInfo.id,
						curretUserInfo.ticket,
						curretUserInfo.roleType.toString(), info.toString(), 1,
						5);
			} catch (ConnectTimeoutException stex) {
			...
			if (!TextUtils.isEmpty(homeWorksInfo)) {
				...
				try {
					if (JSONParser.getIntByTag(messageInfos,
							RestClient.RESULT_TAG_CODE) == RestClient.RESULT_TAG_SUCCESS) {

						campusNotieTotal = JSONParser.getIntByTag(messageInfos,
								"total");
						// try {
						// if(JSONParser.getStringByTag(homeWorksInfo,
						// "datas")!=null){
			...
			Log.i(TAG, "schoolsInfo2 " + messageInfos);

			// TODO: register the new account here.
			return messageInfos;
		}

		private void praseMessageInfos(String messageInfos)
			...
			if (jsonObj.has(RestClient.RESULT_TAG_DATAS) && total > 0) {
				JSONArray data = jsonObj
						.getJSONArray(RestClient.RESULT_TAG_DATAS);
				...
				for (int i = 0; i < data.length(); i++) {
					...
					campusNotice.fkSubjectId = obj.getInt("fkSubjectId");
					campusNotice.smsType = obj.getInt("smsType");
					campusNotice.fkSchoolId = obj.getInt("fkSchoolId");
					// studentInfo.className = obj.getString("id");
					campusNotice.fkStudentId = obj.getInt("fkStudentId ");
		...
		protected void onProgressUpdate(String... result) {
			String update = result[0];
			Log.i(TAG, "onProgressUpdate()");
			Log.i(TAG, "update " + update);
			if ("getHomeWorks".equals(update)) {
				Log.i(TAG, "onProgressUpdate()sss");
			...
			} else if ("getMessageInfos".equals(update)) {
				Log.i(TAG, "onProgressUpdate()vvvv");
		...
		private void parseChildInfo(String childInfo) throws JSONException {
			...
			if (mStudentList.size() > 0) {
				deflutStudentInfo = mStudentList.get(0);
				mSpinnerInfo.clear();
	...
	public class GetAllChildTask extends AsyncTask<Void, Void, String> {
		...
		protected String doInBackground(Void... params) {
			...
			try {
				...
				schoolsInfo = RestClient.getAllChilds(curretUserInfo.id,
						curretUserInfo.fkSchoolId, curretUserInfo.ticket);
			} catch (ConnectTimeoutException stex) {
				schoolsInfo = getString(R.string.request_time_out);
			} catch (SocketTimeoutException stex) {
				schoolsInfo = getString(R.string.server_time_out);
	...
	public void onItemSelected(AdapterView<?> parent, View view, int position,
		...
		if (curretUserInfo.roleType == UserType.ROLE_PARENT) {
			...
			deflutStudentInfo = mStudentList.get(position);
		} else {
	...
	// public class GetCampusNoticeTask extends AsyncTask<Void, Void, String> {
	// @Override
	// protected String doInBackground(Void... params) {
	// // TODO: attempt authentication against a network service.
	// String schoolsInfo = null;
