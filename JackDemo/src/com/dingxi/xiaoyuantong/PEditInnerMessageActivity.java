...
public class PEditInnerMessageActivity extends Activity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		editcancelButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				// TODO Auto-generated method stub
				PEditInnerMessageActivity.this.finish();
			}
		});
		...
		sendHomeWorkButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if(checkText()){
					if (Util.IsNetworkAvailable(PEditInnerMessageActivity.this)) {
						mProgressDialog = new ProgressDialog(
								PEditInnerMessageActivity.this);
						mProgressDialog
	...
	protected boolean checkText() {
		...
		if (curretUserInfo.roleType == UserType.ROLE_PARENT) {
		    if (TextUtils.isEmpty(content)) {
                Toast.makeText(PEditInnerMessageActivity.this, R.string.please_input_content,
                        Toast.LENGTH_LONG).show();
            } else if (TextUtils.isEmpty(mStudentId)){
                Toast.makeText(PEditInnerMessageActivity.this, R.string.please_check_child,
                        Toast.LENGTH_LONG).show();
            } else if (TextUtils.isEmpty(mTeacherId)){
                Toast.makeText(PEditInnerMessageActivity.this, R.string.please_check_teacher,
                        Toast.LENGTH_LONG).show();
            }else {//mPrentId
                isOk = true;
            } 
		    ...
		    if (TextUtils.isEmpty(content)) {
	            ...
	            Toast.makeText(PEditInnerMessageActivity.this, R.string.select_class,
	                    Toast.LENGTH_LONG).show();
	        } else if (TextUtils.isEmpty(mGradeId)) {
	            Toast.makeText(PEditInnerMessageActivity.this, R.string.select_grade,
	                    Toast.LENGTH_LONG).show();
	        } else if (TextUtils.isEmpty(mStudentId)){
	            Toast.makeText(PEditInnerMessageActivity.this, R.string.select_student,
	                    Toast.LENGTH_LONG).show();
	        } else {//mPrentId
	            isOk = true;
	        } 
		...
		protected ResponseMessage doInBackground(Void... params) {
			...
			try {
				...
				if (curretSearchType == SearchType.GradeInfo) {
				 ...
				 responseMessage.praseBody(PEditInnerMessageActivity.this);
			...
			if (responseMessage.code == ResponseMessage.RESULT_TAG_SUCCESS) {
				...
				if (curretSearchType == SearchType.GradeInfo) {
				    if(gradeNameList!=null && gradeNameList.length>0){
				        ...
				        Toast.makeText(PEditInnerMessageActivity.this,
	...
	public class SendHomeWorkTask extends
		...
		protected void onPostExecute(ResponseMessage responseMessage) {
			...
			if (responseMessage.code == ResponseMessage.RESULT_TAG_SUCCESS) {
			    ...
			    mProgressDialog.dismiss();   
			    Intent backIntent = new Intent(PEditInnerMessageActivity.this,LeaveMessageActivity.class);
	...
	public void onClick(View v) {
		// TODO Auto-generated method stub
		if (Util.IsNetworkAvailable(PEditInnerMessageActivity.this)) {
			mProgressDialog = new ProgressDialog(PEditInnerMessageActivity.this);
			...
			if (errorCode != -1) {
				Toast.makeText(PEditInnerMessageActivity.this, errorCode,
						Toast.LENGTH_LONG).show();
